...
Quick note about conventions: although you can plug any gate patterns into LameJuis you want, things become easier to explain/motivate if we assume the top inputs are changing faster than the low inputs.  Then we can say the top inputs represent melodic movement while the middle inputs represent chord progression and bottom inputs represent song form.  This is, of course, just a thing in our brains, but our brains are so used to hearing music organized in this way we might as well exploit this hierarchy.  But from the perspective of how the sequencer actually works, all six inputs are identical.
j
## Logic Matrix
