...
1. **Source Machine**: The sound generator. Currently, this can be **Dual Wave Shaping VCO** (dual wavetable oscillator), **Physical Modeling** (noise-excited SVF + one-pole damping comb), **Thru** (external audio input), or **Dual Sample** (dual sample-directory source using grain resynthesis).
...
- [Source Machine](source-machine.md) — Dual Wave Shaping VCO, Physical Modeling, Thru, and Dual Sample source modes.
