...
The system SHALL convert each incoming 3-byte MIDI message into a `BasicMidi` carrying a microsecond timestamp derived from the JUCE message timestamp (seconds × 1,000,000) and the input handler's route ID, then forward it to the owning integration's `MessageInBus`.
`MidiInputHandler` persists the device name in JSON and reconnects by name via `AttemptConnect()`; messages whose raw size is not 3 bytes are ignored.
...
- **WHEN** an incoming MIDI message has a raw data size other than 3 bytes
...
The system SHALL convert sequencer transport events into MIDI real-time messages on the MidiSender's dedicated clock route: `ProcessMessagesOut` maps `MessageOut::Mode::Clock`, `Start`, and `Stop` to MIDI clock, transport-start, and transport-stop messages stamped with the supplied timestamp, then clears the buffer.
