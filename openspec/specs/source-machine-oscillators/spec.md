...
The Thru source machine SHALL pass the external `SourceMixer` source selected by the voice's source index through the voice, upsampling the source's base-rate u-block by 4 into the oversampled domain so the voice's filter and amp sections shape outside audio.

#### Scenario: External audio is forwarded into the voice chain
- **WHEN** a voice in Thru mode processes a u-block and the selected SourceMixer source carries external input audio
- **THEN** the voice's source output equals the 4x-upsampled external source block, which then feeds the voice's filter machine (whose cutoff anchor in Thru mode is specified in filter-system)
