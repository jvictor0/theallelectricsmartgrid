...
void FillClock(TheoryOfTimeBase& time, TheoryOfTimeBase::Input& clock, double increment = 0.0)
{
...
DOCTEST_TEST_CASE("AbsoluteTime: Nonagon wires source ratio separately from voice gate ratio")
{
    ...
    FillClock(nonagon->m_theoryOfTime, clock);
    ...
    DOCTEST_CHECK(input.m_multiPhasorGateInput.m_voiceCycleRatio[0] == 6);
    ...
    DOCTEST_CHECK(control.m_envelopePeriodSamples == 3000.0);
    ...
    FillClock(nonagon->m_theoryOfTime, clock);
    nonagon->m_theoryOfTime.RolloverMicroblockBuffer();
    DOCTEST_CHECK(envelope.Process(envelopeInput) == doctest::Approx(0.2625));
}
